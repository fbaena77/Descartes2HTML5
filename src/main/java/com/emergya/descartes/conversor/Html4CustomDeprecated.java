...
public class Html4CustomDeprecated {
		...
		if (doc != null && node != null &&
			...
			while(index < children.size()){
				if(!children.get(index).nodeName().equals(Tag.UL.toString())
					&& !children.get(index).nodeName().equals(Tag.OL.toString())){
			...
			for(Node child: appendChild){
				if(child instanceof TextNode){
					...
					if(!StringUtil.isBlank(text.trim())){
						Element liChild = new Element(org.jsoup.parser.Tag.valueOf(Tag.LI.toString()), "");
	...
	public static Boolean addAppletPlugin(Document doc, Element node) {
		...
		final Elements scripts = doc.head().getElementsByTag(Tag.SCRIPT.toString());
		...
		if(!existDescartes){
			final Element descartesJs = new Element(org.jsoup.parser.Tag.valueOf(Tag.SCRIPT.toString()), "");
			...
			final boolean existFile = FileManager.exist(actual.getParent(), JsoupHtml5Service.SCRIPT_DESCARTES);
			
			/** Añadir el fichero de descartes si no existe*/
			if(!existFile){
				try {
					FileManager.copy(Html4CustomDeprecated.class.getResource("/"+JsoupHtml5Service.SCRIPT_DESCARTES).getPath(), 
							actual.getParent(), JsoupHtml5Service.SCRIPT_DESCARTES);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public static Boolean liParent(Document doc, Element node) {
		...
		if(node.tagName().equals(Tag.LI.toString())
			...
			Element ul = new Element(org.jsoup.parser.Tag.valueOf(Tag.UL.toString()),"");
			node.before(ul);
			ul.appendChild(node);
		}
	...
	public static Boolean fontChildStyle(Document doc, Element node) {
		...
		if(node.tagName().equals(Tag.FONT.toString())){
			...
			if(node.children().size()==0){
				Element actual = new Element(org.jsoup.parser.Tag.valueOf(Tag.SPAN.toString()), "");
				...
				for(Element child: children){
					child.attr("style",child.attr("style")+";color:"+color+"; font-family:"+font);
					node.before(child);
				}
				node.remove();
			}
	...
	public static Boolean bChildStyle(Document doc, Element node) {
		...
		if(node.tagName().equals(Tag.B.toString())){
			if(node.children().size()!=0){
				Elements children = node.children();
				for(Element child: children){
					child.attr("style",child.attr("style")+";font-weight:bold");
					node.before(child);
				}
				node.remove();
			}
	...
	public static Boolean idDuplicate(Document doc, Element node) {
		...
		for(Element child: children){
			if(!StringUtil.isBlank(child.id())){
				...
				Elements duplicates = doc.getElementsByAttributeValue("id", child.id());
				if(duplicates.size()>1){
					corregido = true;
					/** Reemplaza el estilo de identificador por una clase*/
					Elements styles = doc.head().getElementsByTag("style");
					for(Element style:styles){
						style.html(style.html().replaceAll("#"+child.id(), "."+child.id()));
					}
					...
					for(Element dupli: duplicates){
						...
						final String idValue = "html5_"+ dupli.id()
								+ "_"+(rd.nextInt(maxRandom-minRandom+1)+minRandom);
